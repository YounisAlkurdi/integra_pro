...
document.addEventListener("DOMContentLoaded", async () => {
        ...
        sidebarList.innerHTML = nodes.map(node => `
            <div class="interview-item p-8 border-b border-white/5 group" onclick="window.viewArchive('${node.id}')" id="archive-${node.id}">
                <div class="flex items-center gap-5">
                    ...
                    <div>
                        <h4 class="text-xs font-black uppercase tracking-tight italic">${node.candidate_name || 'Unknown'}</h4>
                        <div class="flex items-center gap-2 mt-2">
                             <div class="w-1.5 h-1.5 rounded-full bg-emerald-500/40"></div>
                             <p class="text-[8px] font-mono text-white/30 uppercase tracking-[0.2em]">${node.position || 'Unknown'}</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    window.viewArchive = async (nodeId) => {
        ...
        try {
            ...
            // Header Decryption
            document.getElementById('rep-name').innerText = node.candidate_name || 'Unknown';
            document.getElementById('rep-position').innerText = `${node.position || 'Unknown'} • NEURAL NODE`;
            document.getElementById('rep-avatar').innerText = node.candidate_name ? node.candidate_name[0] : '?';
    ...
    function showToast(msg, type) {
        ...
        if (!toast || !indicator || !text) return;
