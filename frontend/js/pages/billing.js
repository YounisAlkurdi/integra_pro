...
function renderStats(invoices) {
    ...
    const nextBillingEl = document.getElementById('stat-next-billing');

    if (lastAmountEl) lastAmountEl.innerText = `$${(last / 100).toFixed(2)}`;
    if (totalInvestedEl) totalInvestedEl.innerText = `$${(total / 100).toFixed(2)}`;
    if (nextBillingEl) nextBillingEl.innerText = nextDate.toLocaleDateString();
}
