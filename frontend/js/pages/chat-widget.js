 ...
 * Refactored for High-Performance Server-Side Streaming (SSE).
 */
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            const endpoint = window.INTEGRA_SETTINGS?.endpoint('/api/agent/chat') || '/api/agent/chat';
            ...
            if (!response.ok) {
                ...
                throw new Error(errData.detail || "Connection Interrupted");
            }

            // Remove typing indicator as we start receiving data
            document.getElementById(typingId)?.remove();
                    ...
                    if (line.startsWith("data: ")) {
                        try {
                            ...
                            if (data.text) {
                                ...
                                if (!aiMessageId) {
                                    aiMessageId = appendMessage(fullAiResponse, "ai");
                                } else {
                                    updateAiMessage(aiMessageId, fullAiResponse);
                                }
                            }
                        } catch (e) {
                            console.warn("Malformed Stream Chunk:", line);
                        }
                    }
                }
            }

            // Post-stream cleanup/events if needed
    ...
    /**
     * Updates an existing AI message div with new cumulative text.
     * Handles card rendering and system events after the full response or at intervals.
     */
    function updateAiMessage(id, text) {
        const msgDiv = document.getElementById(id);
        if (!msgDiv) return;
        
        // During streaming, we don't want to re-render complex cards every token
        // But we want basic markdown and line breaks
        let formatted = text
            .replace(/&/g, "&amp;")
            .replace(/</g, "&lt;")
            .replace(/>/g, "&gt;")
        ...
        // Check for complete markers and render them
        formatted = processIntegraMarkers(formatted);
    ...
    function appendMessage(text, sender, isTyping = false) {
        ...
        if (sender === "ai") {
            msgDiv.innerHTML = processIntegraMarkers(text);
        } else {
    ...
    /**
     * Handles [INTEGRA_UI_CARD: ...] and [INTEGRA_SYSTEM_EVENT: ...]
     */
    function processIntegraMarkers(text) {
        let formatted = text;
        ...
        while ((match = cardRegex.exec(text)) !== null) {
            try {
                ...
                // Not fully parsed yet or error
            }
        }

        // Render INTEGRA_SYSTEM_EVENT signals
