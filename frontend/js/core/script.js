...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (document.getElementById('chart-bars')) initDashboard();
    ...
    if (document.getElementById('pricing-grid')) loadPricing();

    // 7. Enable 3D Motion Protocols
    if (document.querySelector('#pricing .reveal')) init3DHover();
...
async function loadPricing() {
    ...
    try {
        const response = await fetch('/frontend/assets/pricing.json');
        ...
        plans.forEach((plan, index) => {
            ...
            card.innerHTML = `
                ...
                <div class="mb-12 h-20 flex items-baseline">
                    ...
                    ` : `
                        ...
                        <span class="text-6xl font-bold tracking-tighter price-val ${plan.highlight ? 'text-white' : ''}" 
                              ...
                              ${window.isYearly ? plan.yearly.price : plan.monthly.price}
                        </span>
                ...
                <button class="w-full py-4 border text-xs uppercase tracking-widest font-bold transition-all rounded-full ${buttonClasses}" 
                        onclick="window.location.href='/frontend/pages/checkout.html?plan=${plan.id}'">
                    ${plan.button_text}
                </button>
            `;
...
window.toggleBilling = () => {
    ...
    if (window.isYearly) {
        ...
        if(labelMonthly) labelMonthly.style.color = '#64748b'; 
        if(labelYearly) labelYearly.style.color = '#fff';
        ...
        if(labelMonthly) labelMonthly.style.color = '#fff';
        if(labelYearly) labelYearly.style.color = '#64748b';
