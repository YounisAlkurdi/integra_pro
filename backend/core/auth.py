...
async def get_supabase_jwks():
    ...
    cache_key = "auth:jwks_keys"
    cached_keys = integra_cache.get(cache_key)
    if cached_keys:
        return cached_keys
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                integra_cache.set(cache_key, keys, ttl=3600)
                return keys
...
async def get_current_user(authorization: Optional[str] = Header(None)):
    """
    Neural Verification Protocol (V4).
...
async def get_active_subscription(user_id: str):
    ...
    # Using the cached get from our new supabase client
    res = await supabase.get("subscriptions", f"user_id=eq.{user_id}&status=ilike.active&order=created_at.desc", cache_ttl=300)
    sub = res[0] if res else None
    
    if sub and sub.get('plan_id'):
        plan_id = sub['plan_id']
        templates = {
            'starter': {"interviews_limit": 15, "max_duration_mins": 20},
            'professional': {"interviews_limit": 40, "max_duration_mins": 60},
            'enterprise': {"interviews_limit": 9999, "max_duration_mins": 1440},
            'nexus': {"interviews_limit": 50, "max_duration_mins": 60}
        }
...
async def check_lockdown():
    """
    Ensures the system is not in emergency lockdown mode.
    Used as a dependency for sensitive routes.
    """
    if integra_cache.get("system:lockdown"):
        raise HTTPException(
            status_code=503, 
            detail="SYSTEM_LOCKDOWN: Emergency maintenance in progress. Access restricted."
        )
    return True
