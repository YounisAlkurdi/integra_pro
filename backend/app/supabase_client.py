...
class SupabaseClient:
    """
    Unified Async/Sync Supabase Client for Integra with Caching.
    Handles REST API calls with httpx (async) and requests (sync).
    """
    ...
    def request_sync(self, method: str, path: str, body: Optional[Dict[str, Any]] = None, use_cache: bool = False, ttl: int = 300) -> List[Dict[str, Any]]:
        """Synchronous version for auth middleware compatibility."""
        import requests
        if not SUPABASE_URL or not SUPABASE_SERVICE_KEY:
            return []

        cache_key = f"sync:{method}:{path}:{json.dumps(body) if body else ''}"
        if use_cache and method.upper() == "GET":
            cached_val = cache.get(cache_key)
            if cached_val is not None:
                return cached_val
    ...
    def get_sync(self, table: str, query: str = "", use_cache: bool = False, ttl: int = 300) -> List[Dict[str, Any]]:
        path = f"{table}?{query}" if query else table
        return self.request_sync("GET", path, use_cache=use_cache, ttl=ttl)
