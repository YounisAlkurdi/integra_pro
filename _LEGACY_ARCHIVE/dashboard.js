...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    async function fetchSubscription() {
        if (systemState.profile) return systemState.profile.subscription;
    ...
    async function updateDashboardUI() {
        ...
        if (stats) {
            ...
            if (sub && stats.total >= sub.interviews_limit) {
                ...
                document.getElementById('stat-total').classList.remove('text-red-500', 'animate-pulse');
            }
        }

        // 2. Render Interviews List
        ...
        list.innerHTML = nodes.map(s => `
            <div class="group relative bg-white/[0.01] border border-white/5 p-6 rounded-2xl hover:bg-white/[0.03] hover:border-cyan-400/20 transition-all duration-500 overflow-hidden reveal active cursor-pointer" onclick="joinAsHR('${s.room_id}')">
                <div class="flex justify-between items-center gap-6 relative z-10">
                    ...
                    <div class="flex items-center gap-6">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <button onclick="event.stopPropagation(); copyLink('${s.room_id}')" class="p-2 hover:text-cyan-400 transition-all" title="Copy Candidate Link"><i data-lucide="link" class="w-4 h-4"></i></button>
                            <button onclick="event.stopPropagation(); terminateSession('${s.room_id}')" class="p-2 hover:text-red-500 transition-all" title="Terminate Session"><i data-lucide="x" class="w-4 h-4"></i></button>
                            <button onclick="event.stopPropagation(); joinAsHR('${s.room_id}')" class="px-4 py-2 bg-white/5 hover:bg-white text-obsidian text-[8px] font-bold uppercase rounded-lg transition-all hover-target"><i data-lucide="play" class="w-3 h-3 inline mr-1"></i> Join</button>
                        </div>
                    </div>
                </div>
    ...
    async function renderAuditLogs() {
        const auditList = document.getElementById('dashboard-audit-logs');
        if (!auditList) return;
        ...
        try {
            ...
            auditList.innerHTML = logs.map(log => {
                ...
                const color = severityColors[log.severity] || 'text-white/40';

                return `
                    <div class="flex items-center justify-between p-4 bg-white/[0.02] border border-white/5 rounded-2xl hover:bg-white/[0.04] transition-all group">
                        <div class="flex items-center gap-4">
                            <div class="w-2 h-2 rounded-full ${color.replace('text-', 'bg-')} shadow-[0_0_8px_currentColor]"></div>
                            <div>
                                <p class="text-[10px] font-bold tracking-tight text-white/90 group-hover:text-white transition-colors">${log.action}</p>
                                <p class="text-[8px] font-mono text-white/20 uppercase tracking-widest mt-1">${log.target_resource} • ${log.resource_id ? log.resource_id.substring(0, 8) : 'SYSTEM'}</p>
                            </div>
                        </div>
                        <div class="text-right">
                            <p class="text-[9px] font-mono text-white/30">${formatTimeAgo(log.created_at)}</p>
                            <p class="text-[7px] font-mono text-white/10 mt-1 uppercase tracking-widest">${log.ip_address}</p>
                        </div>
                    </div>
                `;
            }).join('');
            ...
            console.error("Audit Render Failure:", e);
        }
    }
    ...
    // --- 7. Quick Protocols ---
    async function triggerSystemProtocol(endpoint, label) {
        showToast(`Initiating ${label}...`, "system");
        try {
            const { data: { session } } = await supabase.auth.getSession();
            const response = await fetch(`${window.APP_CONFIG.backendUrl}/api/system/${endpoint}`, {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                showToast(result.message || `${label} Successful`, "success");
                if (endpoint === 'lockdown') {
                    // Update UI state for lockdown
                    const isLocked = result.locked;
                    window.dispatchEvent(new CustomEvent('integra-system-event', {
                        detail: { 
                            event: 'security-alert', 
                            severity: isLocked ? 'CRITICAL' : 'INFO', 
                            msg: isLocked ? 'EMERGENCY LOCKDOWN ACTIVE' : 'SYSTEM ACCESS RESTORED' 
                        }
                    }));
                }
                renderAuditLogs();
            } else {
                showToast(result.detail || `${label} Failed`, "error");
            }
        } catch (e) {
            console.error(`${label} Protocol Error:`, e);
            showToast("Network Error: Core Unreachable", "error");
        }
    }

    document.getElementById('protocol-reboot')?.addEventListener('click', () => triggerSystemProtocol('reboot', 'Service Reboot'));
    document.getElementById('protocol-prune')?.addEventListener('click', () => triggerSystemProtocol('prune', 'Log Pruning'));
    document.getElementById('protocol-lockdown')?.addEventListener('click', () => triggerSystemProtocol('lockdown', 'Emergency Lockdown'));
    ...
    window.refreshInterviews = async () => {
        ...
        await syncSystemState(true); // Force sync
        await updateDashboardUI();
    };
    ...
    window.addEventListener('integra-system-event', (e) => {
        ...
        if (["node-created", "node-deleted", "matrix-update", "security-alert"].includes(event)) {
