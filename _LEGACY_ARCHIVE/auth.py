...
async def get_current_user(authorization: Optional[str] = Header(None)):
    ...
    try:
        ...
        if alg.startswith("ES"):
            ...
            # Note: Production should use a proper JWK library to match 'kid'
            # For this environment, we'll attempt with the secret if it's a public key
            ...
            # If no local key, we MUST reject in production, but for this specific dev environment
            # we will allow if JWKS is reachable and we have it, otherwise reject.
            # (In a real app, use python-jose or similar for JWKS-based ES256 verification)
